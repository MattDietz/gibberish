...
func Train(trainPath, goodPath, badPath string, ngramSize int) *Model {
	...
	for prevNgram, nextCharCounts := range counts {
		...
		for char, count := range nextCharCounts {
			probs[prevNgram][char] = math.Log(count / float64(contextCounts[prevNgram]))
		}
	}

	goodProbs := calculateProbabilities(goodPath, probs)
	badProbs := calculateProbabilities(badPath, probs)
	...
	return &Model{Threshold: threshold, Probs: probs}
}
...
func (m *Model) Test(input string) (bool, error) {
	prob, err := averageTransitionProbability(input, m.Probs)
	if err != nil {
		return false, err
	}
...
func (m *Model) Probability(input string) (float64, error) {
	return averageTransitionProbability(input, m.Probs)
}
